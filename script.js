...
// Resume Update Utility
// This function helps you easily update the resume file reference
// Usage: updateResumeFile('DEEPAN CHANDRU RESUME 06.pdf', 'February 2025')
function updateResumeFile(newFileName, updateDate) {
    const resumeLink = document.querySelector('.resume-btn');
    const updateDateElement = document.querySelector('.resume-info small:first-child');
    
    if (resumeLink) {
        resumeLink.href = newFileName;
        console.log(`Resume file updated to: ${newFileName}`);
    }
    
    if (updateDateElement && updateDate) {
        updateDateElement.innerHTML = `<i class="fas fa-calendar-alt"></i> Last updated: ${updateDate}`;
        console.log(`Resume date updated to: ${updateDate}`);
    }
}

// Auto-detect latest resume file (if you follow naming convention)
// This will automatically find the highest numbered resume file
function autoUpdateResume() {
    // This is a placeholder function - in a real scenario, you would need
    // server-side code to scan the directory for resume files
    console.log('Auto-update resume function ready. Implement server-side logic to scan directory.');
}

// Make functions globally available for easy use
window.updateResumeFile = updateResumeFile;
window.autoUpdateResume = autoUpdateResume; 